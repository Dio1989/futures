...
class MdApi(MdApi):
    ...
    def __init__(self):
        ...
        if datetime.datetime.now().hour >= 20:
            ...
            tradingday = today        
        self.f = open('D:/FutTickData/md_'+str(tradingday)+'.txt', 'a')        
    ...
    def onRtnDepthMarketData(self, data):
        ...
        #print data['InstrumentID'], data['UpdateTime'], data['LastPrice'], data['Volume'], data['Turnover'], data['OpenInterest'], data['BidPrice1'], data['BidVolume1'], data['AskPrice1'], data['AskVolume1']
        #f.close()
        if data['UpdateTime'] < '20:59:00' and data['UpdateTime'] > '15:00:00':
            pass
        else:
            self.f.write(','.join([str(data['InstrumentID']), str(data['TradingDay']), str(data['ActionDay']), str(data['UpdateTime']), str(data['UpdateMillisec']), str(data['OpenInterest']), str(data['LastPrice']), str(data['AveragePrice']), str(data['Volume']), str(data['Turnover']), str(data['BidPrice1']), str(data['BidVolume1']), str(data['AskPrice1']), str(data['AskVolume1'])]) + '\n')
...
def main():
    ...
    # 注册前置机地址
    if in_config:
        Mdapi.registerFront("tcp://180.169.75.21:41213")
    ...
    # 登陆
    loginReq = {}                           # 创建一个空字典
    loginReq['UserID'] = '80186637'         # 参数作为字典键值的方式传入
    loginReq['Password'] = '819392964'      # 键名和C++中的结构体成员名对应
    ...
    print u'期货合约数:'+str(len(Tdapi.InID))
